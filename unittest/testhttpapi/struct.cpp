...
#define DecodeString(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsString()) { value = tempdom[key].GetString(); } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeInt(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsInt()) { value = tempdom[key].GetInt(); } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeInt64(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsInt64()) { value = tempdom[key].GetInt64(); } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeArraryString(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsArray()) { for (unsigned int i = 0; i < tempdom[key].Size(); ++i) { if (!tempdom[key][i].IsString()) { std::cout << "decode type is not string(key:" << key << ")." << std::endl; return false; } value.push_back(tempdom[key][i].GetString()); } } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeArraryInt(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsArray()) { for (unsigned int i = 0; i < tempdom[key].Size(); ++i) { if (!tempdom[key][i].IsInt()) { std::cout << "decode type is not int(key:" << key << ")." << std::endl; return false; } value.push_back(tempdom[key][i].GetInt()); } } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeArraryDouble(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsArray()) { for (unsigned int i = 0; i < tempdom[key].Size(); ++i) { if (!tempdom[key][i].IsDouble()) { std::cout << "decode type is not double(key:" << key << ")." << std::endl; return false; } value.push_back(tempdom[key][i].GetDouble()); } } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }
#define DecodeDouble(key, tempdom, value, required) if (tempdom.HasMember(key) && tempdom[key].IsDouble()) { value = tempdom[key].GetDouble(); } else if (required) { std::cout << "decode not find required key(key:" << key << ")." << std::endl; return false; }

#define PraseReqJson() Document dom; \
    if (dom.Parse(buf.c_str()).HasParseError()) { std::cout << "prase req failed(error:" << GetParseError_En(dom.GetParseError()) << ")." << std::endl; return false; } \
    if (!ReqHead::decode(dom)) { std::cout << "decode req head failed." << std::endl; return false; }
#define PraseRespJson() Document dom; \
    if (dom.Parse(buf.c_str()).HasParseError()) { std::cout << "prase resp failed(error:" << GetParseError_En(dom.GetParseError()) << ")." << std::endl; return false; } \
    if (!RespHead::decode(dom)) { std::cout << "decode resp head failed." << std::endl; return false; }
...
bool InitEnvReq::decode(const std::string& buf)
{
    ...
    {
        std::cout << "decode not find required key(key:params)." << std::endl;
        return false;
    }
