...
/*
测试结果：
Cmd(gate=Y1, targets=[0], controls=[], rotation=[], desc=y1 q[0], inverse=0)
Cmd(gate=Y1, targets=[1], controls=[], rotation=[], desc=y1 q[1], inverse=0)
test Y1 inverse state:Zero 初始振幅:[[1, 0],[0, 0],[0, 0],[0, 0],] 应用门后的振幅:[[0.5, 0],[0.5, 0],[0.5, 0],[0.5, 0],] 逆操作后的振幅:[[1, 0],[0, 0],[0, 0],[0, 0],] 
Cmd(gate=Y1, targets=[0], controls=[], rotation=[], desc=y1 q[0], inverse=0)
Cmd(gate=Y1, targets=[1], controls=[], rotation=[], desc=y1 q[1], inverse=0)
test Y1 inverse state:Plus 初始振幅:[[0.5, 0],[0.5, 0],[0.5, 0],[0.5, 0],] 应用门后的振幅:[[0, 0],[0, 0],[0, 0],[1, 0],] 逆操作后的振幅:[[0.5, 0],[0.5, 0],[0.5, 0],[0.5, 0],] 
Cmd(gate=Y1, targets=[0], controls=[], rotation=[], desc=y1 q[0], inverse=0)
Cmd(gate=Y1, targets=[1], controls=[], rotation=[], desc=y1 q[1], inverse=0)
test Y1 inverse state:Classical 初始振幅:[[1, 0],[0, 0],[0, 0],[0, 0],] 应用门后的振幅:[[0.5, 0],[0.5, 0],[0.5, 0],[0.5, 0],] 逆操作后的振幅:[[1, 0],[0, 0],[0, 0],[0, 0],] 
测试结论：通过
*/
bool CTestGateY1::TestInv(const InitState::type state)
{
    ...
    osprint << "test Y1 inverse state:" << to_string(state) << " 初始振幅:[";
